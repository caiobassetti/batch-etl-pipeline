...
deduped AS (
  SELECT *,
         row_number() OVER (
           ...
           ORDER BY
             -- prefer the newest batch when collisions exist
             NULLIF(batch_id, '') DESC,
             -- then prefer most recent ingestion within that batch
             _ingested_at DESC
         ) AS rn
  FROM normalized
)
SELECT customer_id, first_name, last_name, email, country_code,
       signup_date, is_marketing_opt_in, _ingested_at, batch_id
FROM deduped
WHERE rn = 1;
